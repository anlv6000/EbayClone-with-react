    ...
    return (React.createElement("aside", { className: `bg-muted flex-shrink-0 border-r transition-all duration-300 ${sidebarOpen ? "w-64" : "w-16"} relative` },
        React.createElement("div", { className: "p-4" },
            React.createElement("div", { className: `flex items-center w-full ${sidebarOpen ? "justify-start gap-2" : "justify-center"} mb-6` },
                React.createElement("div", { className: "bg-primary rounded-md p-1 text-primary-foreground font-bold text-lg w-full text-center" }, sidebarOpen ? "eBay" : "eB")),
            React.createElement("nav", { className: "space-y-1" }, navItems.map((item) => (React.createElement(Link, { key: item.path, to: item.path, className: `flex items-center ${sidebarOpen ? "gap-3 px-3" : "justify-center px-0"} py-2 rounded-md hover:bg-secondary group ${location.pathname === item.path
